...
import (
	...
	"github.com/solikewind/happyeat/dal/model/ent/migrate"
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.MenuSpec, c.Object, c.OperationLog, c.Order, c.OrderItem, c.Settlement,
...
// QueryOrderItems queries the order_items edge of a Menu.
func (c *MenuClient) QueryOrderItems(_m *Menu) *OrderItemQuery {
...
func (c *OrderClient) QueryTable(_m *Order) *TableQuery {
...
// QueryMenu queries the menu edge of a OrderItem.
func (c *OrderItemClient) QueryMenu(_m *OrderItem) *MenuQuery {
	query := (&MenuClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(orderitem.Table, orderitem.FieldID, id),
			sqlgraph.To(menu.Table, menu.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, orderitem.MenuTable, orderitem.MenuColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *TableClient) QueryOrders(_m *Table) *OrderQuery {
	query := (&OrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		...
		Object, OperationLog, Order, OrderItem, Settlement, SpecGroup, SpecItem, Table,
